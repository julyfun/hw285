...
class DQNAgent(nn.Module):
    def __init__(
        ...
        self.critic: DQNCritic = make_critic(observation_shape, num_actions)
        self.target_critic: DQNCritic = make_critic(observation_shape, num_actions)
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        action = torch.randint(0, self.num_actions, (observation.shape[0], )) \
            if np.random.random() < epsilon else self.critic.forward(observation).argmax(dim=1)
        # ENDTODO
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_qa_values = self.target_critic.forward(next_obs) # (b, num_actions)

            if self.use_double_q:
                # TODO(Section 2.5): implement double-Q target action selection
                next_action = self.critic.forward(next_obs).argmax(dim=1) # (b, )
            else:
