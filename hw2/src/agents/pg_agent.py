...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        G = sum(
            [self.gamma ** i * r for (i, r) in enumerate(rewards)]
        )
        return np.full_like(rewards, G)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in range(n - 2, -1, -1):
            res[i] = self.gamma * res[i + 1] + rewards[i]
        return res
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [self._discounted_return(rew) for rew in rewards]
        else:
            # Case 2: in reward-to-go PG, we only use the rewards after timestep t to estimate the Q-value for (s_t, a_t).
            # In other words: Q(s_t, a_t) = sum_{t'=t}^T gamma^(t'-t) * r_{t'}
            # DONE: use the helper function self._discounted_reward_to_go to calculate the Q-values
            q_values = [self._discounted_reward_to_go(rew) for rew in rewards]
